...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (userFromFirebase) => {
            ...
            if (userFromFirebase) {
                ...
                try {
                    ...
                    const backendUser = await getUserFromBackend(userFromFirebase.uid);
                    
                    // Armazenamos o usuário COMPLETO (com a role)
                    setUser(backendUser); 
                } catch (error) {
                    console.error("Falha ao buscar role do backend:", error);
