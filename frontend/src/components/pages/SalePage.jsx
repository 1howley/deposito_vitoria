...
export function SalePage({ onAddToCart, onBack }) {
    ...
    const offersProducts = [
        { id: "o1", name: "Cimento Portland CP-V-ARI 50kg", price: 38.90, originalPrice: 45.90, image: "https://images.unsplash.com/photo-1635789145651-d5770bc33039?w=800", category: "Materiais Básicos", description: "Cimento de alta resistência inicial para estruturas", inStock: true, rating: 4.9, discount: 15 },
        // ... (resto dos seus produtos em oferta) ...
        { id: "o12", name: "Compensado Naval 15mm", price: 159.90, originalPrice: 189.90, image: "https://images.unsplash.com/photo-1647124498956-1ba4742f5f4e?w=800", category: "Madeiras", description: "Compensado naval resistente à umidade", inStock: true, rating: 4.7, discount: 16 }
    ];

    // Categorias baseadas nos produtos em oferta (Sua lógica)
    const categories = [
        { id: "all", name: "Todas as Categorias", count: offersProducts.length },
        { id: "Materiais Básicos", name: "Materiais Básicos", count: offersProducts.filter(p => p.category === "Materiais Básicos").length },
        { id: "Ferramentas", name: "Ferramentas", count: offersProducts.filter(p => p.category === "Ferramentas").length },
        { id: "Tintas e Pintura", name: "Tintas e Pintura", count: offersProducts.filter(p => p.category === "Tintas e Pintura").length },
        { id: "Madeiras", name: "Madeiras", count: offersProducts.filter(p => p.category === "Madeiras").length }
    ];
            ...
            <div className="bg-white border-b sticky top-0 z-40">
                <div className="container mx-auto px-4 py-4 md:py-6">
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6">
                        ...
                        <div className="min-w-0 flex-1"> {/* Removido flex e gap */}
                            {/* O ícone <Percent /> foi removido daqui */}
                            <div>
                                <h1 className="text-xl md:text-3xl font-bold truncate">Ofertas Especiais</h1>
                    ...
                    <div className="space-y-3 md:space-y-0 md:flex md:items-center md:gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 md:hidden">
                            ...
                            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                                ...
                                <SelectContent>
                                    {categories.map(cat => (<SelectItem key={cat.id} value={cat.id}>{cat.name} ({cat.count})</SelectItem>))}
                                </SelectContent>
                            </Select>
                        ...
                        {filteredProducts.map(product => (
                            viewMode === "grid" ? (
                                <div key={product.id} className="relative">
                                    {product.discount && (<div className="absolute top-2 right-2 z-10 bg-accent text-accent-foreground px-2 py-1 rounded-lg text-xs font-bold"> -{product.discount}% </div>)}
                                ...
                                <div key={product.id} className="bg-white rounded-lg border p-4 flex items-center gap-4 hover:shadow-md transition-shadow">
                                    <div className="relative">
                                        ...
                                        {product.discount && (<div className="absolute -top-2 -right-2 bg-accent text-accent-foreground px-2 py-1 rounded-lg text-xs font-bold"> -{product.discount}% </div>)}
                                    </div>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="text-lg font-bold text-primary">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(product.price)}</span>
                                                    {product.originalPrice && (<span className="text-sm text-muted-foreground line-through">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(product.originalPrice)}</span>)}
                                                </div>
                                            </div>
                                            <Button onClick={() => onAddToCart(product)} disabled={!product.inStock} className="shrink-0 w-full sm:w-auto" size="sm"> {product.inStock ? 'Adicionar' : 'Indisponível'} </Button>
                                        </div>
                                    </div>
                                </div>
                            )
